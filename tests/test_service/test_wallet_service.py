...
def test_create_wallet(db_session, current_user):
    ...
    wallet = wallets_service.create_wallet(db_session, current_user=current_user, wallet=payload)
...
def test_create_wallet_exists(db_session, current_user, wallet):
    ...
    with pytest.raises(HTTPException) as exc:
        wallets_service.create_wallet(db_session, current_user=current_user, wallet=payload)
...
def test_get_wallet_total_balance(db_session, current_user):
    ...
    result = asyncio.run(wallets_service.get_total_balance(db_session, current_user=current_user))
...
def test_get_wallet_not_exists(db_session, current_user):
    ...
    with pytest.raises(HTTPException) as exc:
        wallets_service.get_wallet(db_session, current_user=current_user, wallet_name="nonexistent")
...
def test_get_wallet_other_user(db_session, wallet):
    ...
    with pytest.raises(HTTPException) as exc:
        wallets_service.get_wallet(db_session, current_user=other_user, wallet_name=wallet.name)
