...
async def test_add_income(db_session: AsyncSession, current_user, wallet):
    ...
    updated_wallet = await wallets_repository.add_income(
        db_session, user_id=current_user.id, wallet_name=wallet.name, amount=income_amount,
    )
...
async def test_add_expense(db_session: AsyncSession, current_user, wallet):
    ...
    updated_wallet = await wallets_repository.add_expense(
        db_session, user_id=current_user.id, wallet_name=wallet.name, amount=expense_amount,
    )
...
async def test_get_user_wallets(db_session: AsyncSession, current_user):
    wallet1 = Wallet(
        name="wallet1", balance=Decimal("100"), user_id=current_user.id, currency=CurrencyEnum.USD,
    )
    db_session.add(wallet1)

    wallet2 = Wallet(
        name="wallet2", balance=Decimal("200"), user_id=current_user.id, currency=CurrencyEnum.USD,
    )
